...
namespace Alpha_Optical.Areas.Admin.Controllers
{
    [Area("Admin")]
    // authorize for route 
    public class DetailsSectionController : Controller
    {

        private readonly IUnitofWork _UnitofWork;
        private readonly IWebHostEnvironment _WebHostEnvironment;
        public DetailsSectionController(IUnitofWork unitofWork, IWebHostEnvironment webHostEnvironment)
        {
        ...
        {
            return View(await _UnitofWork.DetailsSection.GetAllasync());
        }
        ...
        // post 
        [HttpPost]
        public async Task<IActionResult> Create(DetailsSection header, IFormFile? file)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    var featuretest = await _UnitofWork.DetailsSection.Addasync(header);
                    if (featuretest)
                    {
        ...
        public async Task<IActionResult> Edit(int id)
        {
            ...
            var featuretest = await _UnitofWork.DetailsSection.Getasync(id);
            if (featuretest == null)
            {
                return NotFound();
            }
        ...
        // post Edit
        [HttpPost]
        public async Task<IActionResult> Edit(int id, DetailsSection header, IFormFile? file)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    var featuretest = await _UnitofWork.DetailsSection.Updateasync(header);
                    if (featuretest)
                    {
        ...
        {
            return await _UnitofWork.DetailsSection.HeadersAnyAsync(id);
        }
        ...
        public async Task<JsonResult> JsonDelete(int? id)
        {
            ...
            var result = await _UnitofWork.DetailsSection.Deleteasync(id.Value);
            if (result)
            {
