...
namespace Alpha_Optical.Areas.Admin.Controllers
{
    ...
    public class HeaderController : Controller
    {
        ...
        {
            return View(await _UnitofWork.header.GetAllasync());
        }
        ...
        // post 
        [HttpPost]
        public async Task<IActionResult> Create(Header header, IFormFile? file)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    var featuretest = await _UnitofWork.header.Addasync(header);
                    if (featuretest)
                    {
        ...
        public async Task<IActionResult> Edit(int id)
        {
            ...
            var featuretest = await _UnitofWork.header.Getasync();
            if (featuretest == null)
            {
                return NotFound();
            }
        ...
        // post Edit
        [HttpPost]
        public async Task<IActionResult> Edit(int id, Header header, IFormFile? file)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    var featuretest = await _UnitofWork.header.Updateasync(header);
                    if (featuretest)
                    {
        ...
        {
            return await _UnitofWork.header.HeadersAnyAsync(id);
        }
        ...
        public async Task<JsonResult> JsonDelete(int? id)
        {
            ...
            var result = await _UnitofWork.header.Deleteasync(id.Value);
            if (result)
            {
